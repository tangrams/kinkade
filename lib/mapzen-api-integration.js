...
function addScenefile(scene) {
    ...
    var scenefile = YAML.stringify(window.scene.config, 8);
...
function checkScenefile(result, path) {
    console.log('checkScenefile:', result, path);
...
function receiveScenes(data) {
    ...
    for (var i = 0; i < scenes.length; i++) {
        ...
        // var url = "/api/scenes/"+user.id+"/"+obj.id+"/resources/spheremap.png"
        var url = "/api/scenes/"+user.id+"/"+obj.id+"/resources/spheremap.png";
        ...
        // mzUserAPI.get(url, 'yaml', attachScenefile, obj);
