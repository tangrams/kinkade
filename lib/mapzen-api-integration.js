...
function makePlaceholder(obj) {
    ...
    // test to make sure it's a kinkade scene
    // (can't open Tangram Play scenes for now)
    if (obj.metadata && obj.metadata.kinkade) {
...
function saveScene() {
    ...
    makeThumbnail().then(function(result) {
        var extra = {
            'kinkade': true,
            'terrain_scale' : parseInt(document.getElementById('scale').value),
            'timestamp' : Date.now(),
...
function postScene(data) {
    if (typeof data != 'undefined') {
        ...
        // post new scene and add its spheremap afterward
...
function loadScene(id) {
    ...
    window.map.flyTo({lat: obj.metadata.view_lat, lng: obj.metadata.view_lng}, parseFloat(obj.metadata.view_zoom), {duration: 1.})
    ...
    window.scene.load(obj.entrypoint_url).then(function(result){
        ...
        scene.updateConfig().then(function() {
            // update scale
            document.getElementById('scale_slider').value = obj.metadata.terrain_scale;
            updateScale(obj.metadata.terrain_scale);
