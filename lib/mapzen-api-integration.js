...
function addScenefile(scene) {
    ...
    // edit the original scenefile yaml
    // switch out the link to #kcanvas to the s3 image url when we save it
    var scenetext = window.scenefile.replace('element: "#kcanvas"', 'url: '+url);
    var scaleVal = document.getElementById("scale").value;
    scaleVal = parseFloat(1/(Math.pow(2,scaleVal)-1));
    scenetext = scenetext.replace('u_scale: .5', 'u_scale: '+scaleVal);

    // insert the user's API key
    var userApiKey = document.getElementById("apikey").value;
    // make sure they've customized the key
    if (userApiKey == "mapzen-XXXXXXX") {
        alert("Please enter your API key in the text field provided.");
        document.getElementById("apikey").select();
        return false;
    }
    console.log('userApiKey:', userApiKey);
    scenetext = scenetext.replace('/your\-mapzen\-api\-key/g', userApiKey);
    ...
    mzUserAPI.put(scenefileURL, scenetext, "text/plain", checkScenefile);
...
function addMetadata(scene) {
    ...
    var metadata = {
        ...
        'view_lat' : window.scene.view.center.lat,
        'view_lng' : window.scene.view.center.lng,
        'view_zoom' : window.scene.view.zoom
    }
...
function saveScene() {
    ...
    // scenetext = scenetext.replace(/YOUR/\API\-KEY/g, userApiKey);
    ...
    makeThumbnail().then(function(result) {
        var data = {
            ...
            versions_leaflet: "1.2.0"
        };
        postScene(data);
    });
}

// these match Tangram Play's specs for now (hd ratio)
...
function loadScene(id) {
    ...
    window.map.flyTo({lat: obj.view_lat, lng: obj.view_lng}, parseFloat(obj.view_zoom), {duration: 1.})

    // if there's a scene file available, load it
    window.scene.load(obj.entrypoint_url).then(function(result){
        // default key injections
        ...
        if (typeof scene.config.textures !== 'undefined') {
            if (typeof scene.config.textures.spheremap !== 'undefined') {
                delete(scene.config.textures.spheremap.url);
            } else {
                // there's no spheremap  - kinkade has not been here
                // define the object, set it later
                console.log('empty spheremap')
                scene.config.textures.spheremap = {}
                console.log('scene.config.textures.spheremap:', scene.config.textures.spheremap);
            }
        } else {
            scene.config.textures = {};
            scene.config.textures.spheremap = {};
        }
        if (typeof scene.config.scene === 'undefined') {
            scene.config.scene = {};
        }
        if (typeof scene.config.styles === 'undefined') {
            scene.config.styles = {};
        }
        delete scene.config.textures.spheremap.url;
        scene.config.textures.spheremap.element = '#kcanvas';
        ...
        scene.updateConfig().then(function() {
            // update scale
            document.getElementById('scale_slider').value = obj.terrain_scale;
            updateScale(obj.terrain_scale);
