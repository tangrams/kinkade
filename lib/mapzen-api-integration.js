...
function addMetadata(scene) {
    ...
    url = '/api/scenes/'+user.id+'/'+scene.id+'/resources/metadata.json';
    // add the metadata to the scene in question, then refetch the scenes list
    put(url, JSON.stringify(metadata), "application/json", checkMetadata);
}
...
function receiveScenes(data) {
    ...
    for (i = 0; i < scenes.length; i++) {
        ...
        get(url, 'json', applyMetadata, obj);
...
function postScene(data) {
    if (typeof data != 'undefined') {
        ...
        post('/api/scenes/'+user.id, data, scenePosted);
    }
}
...
function clearScenes() {
    get('/api/scenes/'+user.id, 'json', function(scenes) {
        for (var i = 0; i< scenes.length; i++) {
            document.getElementById("scenes").removeChild(document.getElementById(scenes[i].id));
            deletePath('/api/scenes/'+user.id+'/'+scenes[i].id);
        }
    });
}
